...
class TopicModelFactories {
	static def oneTopic() { new Topic("oneRandomTopicName", false) }

	static def oneTopicDetail(port = 0) {
		new TopicDetail(new Topic("oneRandomTopicName", false), 1, [
	...
	static def anotherTopic() { new Topic("anotherRandomTopicName", false) }
	...
	static def topicListings() {
		[new TopicListing(oneTopic().name, oneTopic().internal), new TopicListing(anotherTopic().name, anotherTopic().internal)]
	}
}
